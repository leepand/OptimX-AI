...
from optimx.web import fromtimestamp
...
class PsDashRunner(object):
    ...
    def _get_args(cls, args):
        ...
        parser.add_argument(
            ...
            default=None,
            metavar='path',
            help='log files to make available for optimx. Patterns (e.g. /var/log/**/*.log) are supported. '
        ...
        parser.add_argument(
            ...
            default=None,
            metavar='host:port',
            help='The optimx node running in web mode to register this agent to on start up. e.g 10.0.1.22:5000'
        )
        parser.add_argument(
            '--register-as',
            action='store',
    ...
    def _setup_nodes(self):
        ...
        nodes = self.app.config.get('OPTIMX_NODES', [])
    ...
    def _create_app(self, config=None):
        ...
        from optimx.web import webapp
        prefix = app.config.get('OPTIMX_URL_PREFIX')
        if prefix:
            prefix = '/' + prefix.strip('/')
    ...
    def _load_allowed_remote_addresses(self, app):
        key = 'OPTIMX_ALLOWED_REMOTE_ADDRESSES'
    ...
    def _setup_logging(self):
        level = self.app.config.get('OPTIMX_LOG_LEVEL', logging.INFO) if not self.app.debug else logging.DEBUG
        format = self.app.config.get('OPTIMX_LOG_FORMAT', '%(levelname)s | %(name)s | %(message)s')
    ...
    def _setup_workers(self):
        net_io_interval = self.app.config.get('OPTIMX_NET_IO_COUNTER_INTERVAL', self.DEFAULT_NET_IO_COUNTER_INTERVAL)
        gevent.spawn_later(net_io_interval, self._net_io_counters_worker, net_io_interval)

        if 'OPTIMX_LOGS' in self.app.config:
            logs_interval = self.app.config.get('OPTIMX_LOGS_INTERVAL', self.DEFAULT_LOG_INTERVAL)
            gevent.spawn_later(logs_interval, self._logs_worker, logs_interval)

        if self.app.config.get('OPTIMX_AGENT'):
            register_interval = self.app.config.get('OPTIMX_REGISTER_INTERVAL', self.DEFAULT_REGISTER_INTERVAL)
    ...
    def _setup_context(self):
        ...
        if 'OPTIMX_LOGS' in self.app.config:
            self.get_local_node().logs.add_patterns(self.app.config['OPTIMX_LOGS'])
    ...
    def _register_agent(self):
        register_name = self.app.config.get('OPTIMX_REGISTER_AS')
        if not register_name:
            register_name = socket.gethostname()

        url_args = {
            'name': register_name,
            'port': self.app.config.get('OPTIMX_PORT', self.DEFAULT_PORT),
        }
        register_url = '%s/register?%s' % (self.app.config['OPTIMX_REGISTER_TO'], urllib.urlencode(url_args))

        if 'OPTIMX_AUTH_USERNAME' in self.app.config and 'OPTIMX_AUTH_PASSWORD' in self.app.config:
            auth_handler = urllib2.HTTPBasicAuthHandler()
            auth_handler.add_password(
                realm='psDash login required',
                uri=register_url,
                user=self.app.config['OPTIMX_AUTH_USERNAME'],
                passwd=self.app.config['OPTIMX_AUTH_PASSWORD']
            )
    ...
    def _run_rpc(self):
        ...
        if 'OPTIMX_REGISTER_TO' in self.app.config:
            self._register_agent()

        service = self.get_local_node().get_service()
        self.server = zerorpc.Server(service)
        self.server.bind('tcp://%s:%s' % (self.app.config.get('OPTIMX_BIND_HOST', self.DEFAULT_BIND_HOST),
                                          self.app.config.get('OPTIMX_PORT', self.DEFAULT_PORT)))
        self.server.run()
    ...
    def _run_web(self):
        ...
        ssl_args = {}
        if self.app.config.get('OPTIMX_HTTPS_KEYFILE') and self.app.config.get('OPTIMX_HTTPS_CERTFILE'):
            ssl_args = {
                'keyfile': self.app.config.get('OPTIMX_HTTPS_KEYFILE'),
                'certfile': self.app.config.get('OPTIMX_HTTPS_CERTFILE')
            }

        listen_to = (
            self.app.config.get('OPTIMX_BIND_HOST', self.DEFAULT_BIND_HOST),
            self.app.config.get('OPTIMX_PORT', self.DEFAULT_PORT)
        )
    ...
    def run(self):
        logger.info('Starting optimx v%s' % __version__)
