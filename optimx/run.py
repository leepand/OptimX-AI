...
class PsDashRunner(object):
    ...
    def _create_app(self, config=None):
        ...
        app.add_template_filter(fromtimestamp2)
    ...
    def _setup_logging(self):
        level = (
            self.app.config.get("OPTIMX_LOG_LEVEL", logging.INFO)
            if not self.app.debug
            else logging.DEBUG
        )
        format = self.app.config.get(
            "OPTIMX_LOG_FORMAT", "%(levelname)s | %(name)s | %(message)s"
        )
    ...
    def _setup_workers(self):
        net_io_interval = self.app.config.get(
            "OPTIMX_NET_IO_COUNTER_INTERVAL", self.DEFAULT_NET_IO_COUNTER_INTERVAL
        )
        gevent.spawn_later(
        ...
        if "OPTIMX_LOGS" in self.app.config:
            logs_interval = self.app.config.get(
                "OPTIMX_LOGS_INTERVAL", self.DEFAULT_LOG_INTERVAL
            )
        ...
        if self.app.config.get("OPTIMX_AGENT"):
            register_interval = self.app.config.get(
                "OPTIMX_REGISTER_INTERVAL", self.DEFAULT_REGISTER_INTERVAL
            )
            gevent.spawn_later(
    ...
    def _register_agent(self):
        ...
        register_url = "%s/register?%s" % (
            self.app.config["OPTIMX_REGISTER_TO"],
            urllib.parse.urlencode(url_args),
        )

        if (
            "OPTIMX_AUTH_USERNAME" in self.app.config
            and "OPTIMX_AUTH_PASSWORD" in self.app.config
        ):
            auth_handler = urllib2.HTTPBasicAuthHandler()
            auth_handler.add_password(
                realm="OptimX login required",
                uri=register_url,
    ...
    def _run_web(self):
        ...
        ssl_args = {}
        if self.app.config.get("OPTIMX_HTTPS_KEYFILE") and self.app.config.get(
            "OPTIMX_HTTPS_CERTFILE"
        ):
            ssl_args = {
