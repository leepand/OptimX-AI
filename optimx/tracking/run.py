...
from .opts import Options, options
from .opts import options as global_options
from .storage.database import next_uuid
from .utils.bunch import Bunch
from .utils.exceptions import ExceptionWithMessage, InvalidInput, exception_message
...
class Run:
    ...
    def sysmon(self) -> Any:
        try:
            ...
            from .utils.sysmon import SystemMonitor
    ...
    def datastream_client(self) -> Any:
        try:
            # Importing here to avoid circular import error.
            from .storage.datastream import DataStreamClient
    ...
    def execute_func(self, steps=None, config: dict = None, options: Options = None):
        """
        Return callable that executes self.run(steps=steps, config=config, options=options).
        """
        return partial(
            lambda run: run.execute(steps=steps, config=config, options=options), self
        )

    def execute(self, steps=None, config: dict = None, options: Options = None):
        """
        ...
        random_seed = (
            hash(self.id_run) + global_options().get("reproducibility.random_seed")
        ) % 2**32 - 1
        ...
        cm_datastream_client = (
            nullcontext()
            if options.get("datastream.disable")
            else self.datastream_client()
        )
    ...
    def df(self, max_level: int = 0) -> pd.DataFrame:
        ...
        df = pd.json_normalize(
            self.fields | {"id_run": self.id_run}, max_level=max_level
        )
        return reorder_columns(df, ["id_run"])


def normalize_steps(steps) -> List[Callable]:
    """
