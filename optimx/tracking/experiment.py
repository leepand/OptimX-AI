...
from .storage import models, serialization
from .storage.archivestore import ArchiveStoreIO
...
from .storage.datastore import DataStoreIO
from .utils.bunch import Bunch
from .utils.enums import IfExists, enforce_enum
from .utils.exceptions import ExceptionWithMessage, InvalidInput
from .utils.frames import reorder_columns
...
class ExperimentAlreadyExists(ExceptionWithMessage):
    ...
    def __init__(self, name):
        return super().__init__(
            f"Experiment '{name}'already exists, pass if_exists=\"replace\" to overwrite."
        )
...
class Experiment:
    ...
    def get_tablename(self) -> str:
        ...
        return sanitize_table_name(
            f"{options().get('database.experiment_tableprefix')}{self.name}"
        )
    ...
    def load_runs(self, meta: dict) -> Experiment:
        ...
        def series_to_run(row: pd.Series) -> Run:
            ...
            """
            fields = row[
                meta.runs.columns.serialized + meta.runs.columns.non_serialized
            ].to_dict()
    ...
    def copy_to(
        self, name: str | None = None, db: Database | None = None
    ) -> Experiment:
        """
    ...
    def load(
        ...
        db: Database,
        name: str | None = None,
        id_experiment: uuid.UUID | None = None,
        ...
        with db.session() as session:
            ...
            with options().ctx(
                {"archivestore.relative_path_prefix": str(record.id_experiment)}
            ):
    ...
    def record(
        ...
        store_unsafe_pickle = options().default_if_null(
            store_unsafe_pickle, "serialization.store_unsafe_pickle"
        )
        ...
        return self.model_cls(
            ...
            unsafe_pickle=(
                serialization.unsafe_pickle(self) if store_unsafe_pickle else None
            ),
    ...
    def get_metadata(self) -> dict:
        ...
        meta.optimxtra = Bunch()
        meta.optimxtra.version = __version__
        return meta
    ...
    def persist(
        ...
        with options().ctx(
            ...
            with self.db.session() as session:
                session.add(
                    self.record(meta=meta, store_unsafe_pickle=store_unsafe_pickle)
                )
                session.commit()

            # Create and insert rows in "experiment_..." table.
            df_runs, dtype = serialization.runs_to_sql(
                self.id_experiment, meta, self.runs
            )
            self.db.pandas_to_sql(
                df_runs, meta.runs.table_name, if_exists.name, dtype=dtype
            )
        ...
        with self.db.session() as session:
            if session.query(self.model_cls).filter_by(name=self.name).count() > 0:
                ...
                if if_exists in [IfExists["replace"], IfExists["delete"]]:
                    ...
                    session.query(Experiment.model_cls).filter(
                        Experiment.model_cls.name == self.name
                    ).delete()
                else:
    ...
    def df(self, max_level=0) -> pd.DataFrame:
        ...
        #    max_level=max_level,
        # )
        fields_dict = self.fields.copy()
        fields_dict.update({"id_experiment": self.id_experiment, "name": self.name})

        df_experiment = pd.json_normalize(fields_dict, max_level=max_level)
    ...
    def ls(cls, db: Database) -> pd.DataFrame:
        ...
        with db.session() as session:
            ...
            df_experiments["table_name"] = df_experiments["name"].apply(
                lambda name: sanitize_table_name(
                    f"{options().get('database.experiment_tableprefix')}{name}"
                )
            )
