...
import optimx.tracking as optimxtra
from optimx.tracking.experiment import Experiment
from optimx.tracking.opts import options
from optimx.tracking.storage.datastream import datastream_server
from optimx.tracking.utils.base_options import add_option_argument
from optimx.tracking.utils.exceptions import InvalidInput
from optimx.tracking.utils.logging import init_logging
...
def main():
    ...
    main_parser = argparse.ArgumentParser(
        description="OptimxTra CLI tool.", add_help=False
    )
    ...
    subparsers.add_parser(
        "dss", help="Start the DataStream server", parents=[main_parser]
    )
    subparsers.add_parser("ls", help="List experiments", parents=[main_parser])

    # Command "exp"
    parser_cmd = subparsers.add_parser(
        "exp", help="Show experiment", parents=[main_parser]
    )
    ...
    # Command "runs"
    parser_cmd = subparsers.add_parser(
        "runs", help="Show experiment's runs", parents=[main_parser]
    )
    group = parser_cmd.add_mutually_exclusive_group(required=True)
    group.add_argument("--uuid", dest="experiment_uuid")
    group.add_argument("--name", dest="experiment_name")
    ...
    # Command "stats"
    parser_cmd = subparsers.add_parser(
        "stats", help="Show experiment's stats", parents=[main_parser]
    )
    ...
    # Command "options"
    parser_cmd = subparsers.add_parser(
        "options", help="Show options", parents=[main_parser]
    )
    ...
    if args.cmd == "dss":
        ...
        session = optimxtra.create_session()
        print_df(session.ls())
    elif args.cmd == "exp":
        session = optimxtra.create_session()
        experiment = session.load_experiment(
            name=args.experiment_name, id_experiment=args.experiment_uuid
        )
    ...
    for name, value in experiment.runs.first().fields.items():
        ...
        if isinstance(value, pd.DataFrame):
            if value.shape[0] == 0:
                ...
                stats[f"field.{name}.columns"] = [(k, type(v)) for k, v in row.items()]
                stats[f"field.{name}.n_rows"] = sum(
                    [run.fields[name].shape[0] for run in experiment.runs.values()]
                )
