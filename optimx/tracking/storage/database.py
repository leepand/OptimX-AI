...
from optimx.tracking.storage.models import Base
from optimx.tracking.utils.bunch import Bunch
from optimx.tracking.utils.enums import IfExists
from optimx.tracking.utils.exceptions import InvalidInput
from optimx.tracking.utils.web import fetch
from optimx.utils.file_utils import path_to_local_sqlite_uri
    ...
    def __init__(
        ...
        if url is not None:
            if not url.startswith("sqlite://"):
                url = path_to_local_sqlite_uri(url)
        echo = options().default_if_null(echo, "database.echo")
        pool_pre_ping = options().default_if_null(
            pool_pre_ping, "database.pool_pre_ping"
        )
        self.params = Bunch(
            url=url, ask_password=ask_password, echo=echo, pool_pre_ping=pool_pre_ping
        )
        ...
        self.engine = create_engine(
            self.url, echo=self.params.echo, pool_pre_ping=self.params.pool_pre_ping
        )
    ...
    def pandas_to_sql(
        ...
        with self.session() as session:
            if len(df) == 0 or options().get("tqdm.disable"):
                ...
                df.to_sql(
                    name, session.bind, if_exists=if_exists, index=False, dtype=dtype
                )
            else:
                ...
                for idx, df_chunk in enumerate(dfs):
                    ...
                    def process_chunk(df_chunk, idx):
                        df_chunk.to_sql(
                            name,
                            session.bind,
                            if_exists=if_exists if idx == 0 else "append",
                            index=False,
                            dtype=dtype,
                        )
...
def pandas_query(
    ...
    df_chunks_iterator = pd.read_sql_query(
        ...
        chunksize=options().get("database.query_read_chunk_size"),
    )
...
def next_uuid(seed: int = None, inc: int = 1) -> uuid.UUID:
    """
...
def tqdm_chunks(
    ...
    options_dict = options().get("tqdm") or {}
    options_dict.update({"total": total})
    pbar = tqdm(**options_dict)
...
def hash_uuid(value: uuid.UUID) -> str:
    """
...
def query_count(query, session: Session):
    """
    Returns the number of rows returned if executing `query`.
    """

    query = normalize_query(query)
    return session.execute(text(f"SELECT COUNT(*) FROM ({query})")).first()[
        0
    ]  # NOQA S608
