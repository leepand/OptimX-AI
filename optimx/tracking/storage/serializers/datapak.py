...
from optimx.tracking.storage.archivestore import Archive, ArchiveStore
from optimx.tracking.storage.datastore import DataStore
from optimx.tracking.storage.serializers.pickle import PickleSerializer
from optimx.tracking.storage.serializers.serializer import Serializer
from optimx.tracking.utils.bunch import Bunch
from optimx.tracking.utils.exceptions import ExceptionWithMessage
...
KEY_BUNCH_0 = "optimxtra.Bunch-0"
KEY_SEQUENCE_0 = "optimxtra.Sequence-0"
KEY_DATASTORE_0 = "optimxtra.DataStore-0"
KEY_ARCHIVE_0 = "optimxtra.Archive-0"
KEY_ARCHIVESTORE_0 = "optimxtra.ArchiveStore-0"
...
def ensure_bytes(data: bytes) -> bytes:
    ...
    if not isinstance(data, bytes):
        raise EncodingError(
            f"Trying to return non-bytes as encoded object ({data.__class__}), this should not happen."
        )
...
class DataPakSerializer(Serializer):
    ...
    def decode(cls, obj: object) -> object:
        ...
        if isinstance(obj, set):
            ...
            raise UnsupportedObjectType(
                f"{cls.__class__} is unable to deserialize type {obj.__class__}"
            )
...
def encode_magic_key(cls, obj: object) -> dict:  # NOQA C901
    ...
    if isinstance(obj, Sequence):
        ...
        return {
            KEY_MAGIC: KEY_PANDAS_DATAFRAME_0,
            "value": ensure_bytes(buffer.getvalue()),
        }
    elif isinstance(obj, pd.Series):
        buffer = BytesIO()
        write_feather(obj.to_frame(), buffer, compression="uncompressed")
        return {
            KEY_MAGIC: KEY_PANDAS_SERIES_0,
            "value": ensure_bytes(buffer.getvalue()),
        }
        ...
        return {
            KEY_MAGIC: KEY_NUMPY_NDARRAY_0,
            "value": ensure_bytes(buffer.getvalue()),
        }
    elif isinstance(obj, np.datetime64):
        return {
            KEY_MAGIC: KEY_NUMPY_DATETIME64_0,
            "value": int(np.datetime64(obj, "us").astype(np.int64)),
        }
    else:
        raise UnsupportedObjectType(
            f"{cls.__name__} does not support type {obj.__class__}"
        )
...
def decode_magic_key(cls, obj: dict) -> object:  # NOQA C901
    ...
    if obj[KEY_MAGIC] == KEY_SEQUENCE_0:
        ...
        raise UnsupportedObjectType(
            f"{cls.__class__} does not support type {obj[KEY_MAGIC]}"
        )
