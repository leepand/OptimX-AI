...
from optimx.tracking.experiment import ExperimentNotFoundException
from optimx.tracking.opts import options
from optimx.tracking.run import Run
from optimx.tracking.session import Session
from optimx.tracking.storage import serialization
from optimx.tracking.storage.database import Database
from optimx.tracking.utils.bunch import Bunch
from optimx.tracking.utils.enums import enforce_enum
from optimx.tracking.utils.exceptions import (
...
class DataStreamServer:
    ...
    def start(self, blocking=False) -> DataStreamServer:
        ...
        if self.kind == DataStreamKind["UNIX"]:
            ...
            self.address = resolve_inet_address(
                options().get("datastream.srv_address").split(":")
            )
            self.bufsize = 1500
        else:
        ...
        if self.kind == DataStreamKind["UNIX"]:
            try:
                ...
                log.warning(
                    f"{self.__class__.__name__}: Relinked stale '{self.address}'"
                )
            except OSError:
                pass
        ...
        log.debug(
            f"Listening on '{options().get('datastream.srv_address')}' ({self.kind.name})"
        )
...
class DataStreamClient:
    ...
    def __init__(
        ...
        if self.kind == DataStreamKind["UNIX"]:
            ...
            self.address = resolve_inet_address(
                options().get("datastream.cli_address").split(":")
            )
            self.bufsize = 1500
        else:
        ...
        log.debug(
            f"Streaming to '{options().get('datastream.cli_address')}' ({self.kind.name})"
        )
        ...
        for message in self.batch:
            try:
                ...
                if message["id_experiment"] not in self.experiments:
                    self.experiments[message["id_experiment"]] = (
                        self.session.load_experiment(
