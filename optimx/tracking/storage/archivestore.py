...
from optimx.tracking.storage.datastore import DataStoreIO
from optimx.tracking.utils.bunch import Bunch
from optimx.tracking.utils.fs import globs
...
class Archive:
    ...
    def extract(self, target: str = ".", members: list[str] | None = None):
        ...
        with tarfile.open(fileobj=buffer, mode="r") as archive:
            ...
            filtered_members = [member for member in members if "data" in member.name]
            archive.extractall(path=target, members=filtered_members)
...
class ArchiveStoreIO:
    ...
    def add_files(
        ...
        with tarfile.open(
            fileobj=fileobj,
            mode=options().get("archivestore.mode"),
            format=options().get("archivestore.format"),
        ) as archive:

            for idx, glob_name in enumerate(
                globs(src_dir, include=include, exclude=exclude)
            ):
    ...
    def extract(
        self, target: str | None = None, members: list[str] | None = None
    ) -> ArchiveStoreIO:
        """
        Extracts the archive to `target` directory.
        """
...
class ArchiveStore:
    ...
    def __init__(
        ...
        self.params = Bunch(
            src_dir=src_dir, arc_dir=arc_dir, include=include, exclude=exclude
        )
