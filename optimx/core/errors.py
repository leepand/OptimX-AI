...
class OptimxDataValidationException(Exception):
    def __init__(
        self,
...
class ReturnValueValidationException(OptimxDataValidationException):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
...
class ItemValidationException(OptimxDataValidationException):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args,
...
def is_optimx_internal_frame(frame: types.FrameType):
    """
    Guess whether the frame originates from a submodule of `optimx`
    """
    try:
        mod = inspect.getmodule(frame)
        if mod:
            frame_package = __package__.split(".")[0]
            return frame_package == "optimx"
    except BaseException:
        pass
    return False


def strip_optimx_traceback_frames(exc: BaseException):
    """
    Walk the traceback and remove frames that originate from within optimx
    Return an exception with the filtered traceback
    """
    tb = None
    for tb_frame, _ in reversed(list(traceback.walk_tb(exc.__traceback__))):
        if not is_optimx_internal_frame(tb_frame):
...
def wrap_optimx_exceptions(func: T) -> T:
    ...
    def wrapper(*args, **kwargs):
        if kwargs.pop("__internal", False):
            ...
            try:
                ...
                if os.environ.get("OPTIMX_ENABLE_SIMPLE_TRACEBACK", "True") == "True":
                    raise strip_optimx_traceback_frames(exc.exc) from exc
...
def wrap_optimx_exceptions_gen(func: T) -> T:
    @functools.wraps(func)
    def wrapper(*args, **kwargs):
        if kwargs.pop("__internal", False):
...
def wrap_optimx_exceptions_async(func: T) -> T:
    @functools.wraps(func)
...
def wrap_optimx_exceptions_gen_async(func: T) -> T:
    @functools.wraps(func)
    async def wrapper(*args, **kwargs):
        if kwargs.pop("__internal", False):
