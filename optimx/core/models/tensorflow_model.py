...
from optimx.core.model import AsyncModel, Model
from optimx.core.types import ItemType, ReturnType
...
class TensorflowModelMixin(abc.ABC):
    ...
    def __init__(self, *args, **kwargs):
        if not has_tensorflow:
            raise ImportError("Tensorflow is not installed, instal optimx[tensorflow].")
...
def tf_serving_retry_policy(name):
    return {
        ...
        "stop": stop_after_attempt(
            int(os.environ.get("OPTIMX_TF_SERVING_ATTEMPTS", 10))
        ),
