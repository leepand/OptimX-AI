...
from optimx.assets.drivers.abc import StorageDriver

try:
    from optimx.assets.drivers.azure import (
        AzureStorageDriver,
    ...
    has_az = False
try:
    from optimx.assets.drivers.gcs import GCSStorageDriver, GCSStorageDriverSettings

    has_gcs = True
except:
    has_gcs = False
from optimx.assets.drivers.local import LocalStorageDriver, LocalStorageDriverSettings

try:
    from optimx.assets.drivers.s3 import S3StorageDriver, S3StorageDriverSettings

    has_s3 = True
except:
    has_s3 = False
from optimx.assets.settings import AssetSpec
from optimx.utils.logging import ContextualizedLogging
...
class StorageProvider:
    ...
    def __init__(
        ...
        **driver_settings,
        ...
        self.timeout = timeout_s or int(os.environ.get("OPTIMX_STORAGE_TIMEOUT_S", 300))
        self.prefix = (
            prefix or os.environ.get("OPTIMX_STORAGE_PREFIX") or "optimx-assets"
        )
        self.force_download = force_download or bool(
            os.environ.get("OPTIMX_STORAGE_FORCE_DOWNLOAD")
        )

        provider = provider or os.environ.get("OPTIMX_STORAGE_PROVIDER")
        if not provider:
        ...
        if provider == "gcs":
            ...
            if not has_az:
                raise DriverNotInstalledError(
                    "Azure driver not installed, install optimx[assets-az]"
                )
