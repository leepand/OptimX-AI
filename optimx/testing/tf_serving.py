...
from optimx.core.models.tensorflow_model import TensorflowModel, connect_tf_serving
    ...
    if deployment == "process":  # pragma: no cover
        ...
        proc = subprocess.Popen(
            [
                ...
                "--model_config_file="
                f"{os.environ['OPTIMX_ASSETS_DIR']}/testing.config",
            ]
            + cmd
        )
        ...
        subprocess.call(
            ["docker", "rm", "-f", "optimx-tfserving-tests"],
        ...
        tfserving_proc = subprocess.Popen(
            [
                ...
                "run",
                "--name",
                "optimx-tfserving-tests",
                "--volume",
                f"{os.environ['OPTIMX_ASSETS_DIR']}:/config",
                "-p",
                "8500:8500",
                "-p",
                "8501:8501",
        ...
        def finalize():
            subprocess.call(["docker", "kill", "optimx-tfserving-tests"])
