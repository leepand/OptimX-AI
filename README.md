...
OptimX-AI是一个用于开发、部署和维护在线强化学习模型的框架。它支持实时更新模型，即在接收到反馈时立即更新模型，并在下一个请求中使用新的模型。在这个框架中，我们的应用场景主要使用小型模型，为每个用户构建独立的模型，并通过交互和优化来不断改进模型的决策能力，它的目标是弥合算法在不同业务场景之间的差距。使用OptimX-AI，您可以确保相同的代码将在生产环境、开发环境或本地的数据处理流水线上运行。
...
一般来说，有几个属性在加载时Model设置：ModelLibrary

- asset_path: Model 的配置中资产集的路径
- configuration_key: 模型配置的关键
- model_dependencies: Model依赖关系字典
- model_settings: model_settings初始化时传递的参数
- service_settings: ModelLibrary创建模型的设置
- batch_size模型的批量大小: 如果_predict_batch实现，它将默认获取此大小的批量。默认为None，表示“无批处理”

#### 模型加载

模型可以实现在通过加载或实例化它时_load调用的方法。这允许您从本地存储（或从对象存储检索）的文件或文件夹加载信息。它们可以包含任意文件、文件夹、参数、优化的数据结构或任何东西。

optimx确保它只被调用一次，无论您想使用模型多少次。

optimx指这些用于将模型加载为资产的支持文件和目录。
